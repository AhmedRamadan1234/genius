            ...
            <div class="row align-items-center">
                <div class="col-lg-6">
                    <div class="w_content_two">
                        <?php
                        if ( !empty($settings['f_image4']['url']) ) { ?>
                            <style>
                                .wave_one_section_three .w_content_two:before {
                                    background: url( <?php echo esc_url($settings['f_image4']['url']) ?> ) no-repeat scroll center left;
                                }
                            </style>
                            <?php
                        }
                ...
                <div class="col-lg-6">
                    <div class="w_phone_img">
                        ...
                        if( !empty( $settings['f_image1']['id'] ) ) {
                            echo wp_get_attachment_image( $settings['f_image1']['id'], 'full', false, array( 'class' => 'p_absoulte dot' ) );
                        }

                        if( !empty( $settings['f_image2']['id'] ) ) {
                            echo wp_get_attachment_image( $settings['f_image2']['id'], 'full', false, array( 'class' => 'p_one' ) );
                        }

                        if( !empty( $settings['f_image3']['id'] ) ) {
                            echo wp_get_attachment_image( $settings['f_image3']['id'], 'full', false, array( 'class' => 'p_two p_absoulte' ) );
                        }
                        ?>
                    </div>
                </div>
