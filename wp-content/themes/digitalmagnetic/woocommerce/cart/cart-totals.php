...
<ul class="shop_table list-unstyled">
	...
	<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
        <li class="shipping">
            <span> <?php esc_html_e( 'Shipping ', 'wavee' ) ?> </span>
	...
	<?php if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) :
		...
		$estimated_text  = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()
			? sprintf( ' <small>' . __( '(estimated for %s)', 'wavee' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] )
			: '';
